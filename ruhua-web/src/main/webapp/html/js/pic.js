...
var mainCutter= null;
var wrapper= null;
var component_box= null;
var processBar= null;
var processPoint= null;
var ctx = null;//ctx.drawImage(myImage, 50, 50);
function previewInImage (file) {
    Options.width = document.body.clientWidth;
    Options.height = document.body.clientHeight - 50;
    ctx = _cropper.getContext('2d');//ctx.drawImage(myImage, 50, 50);
    //通过file.size可以取得图片大小
...
function _initCropAndCut(){
    ...
    $(cutBox).css({"display":"block","width":Options.cutViewWidth+"px","height":Options.cutViewHeight+"px","left":Options.cutLeft+"px","top":Options.cutTop+"px"});
...
$(function(){
    img_preview = document.getElementById("imgPreview");
    cutBox=document.getElementById("cutBox");
    tipBox=document.getElementById("tipBox");
    _cropper=document.getElementById("cropper");
    mainCutter=document.getElementById("mainCutter");
    wrapper=document.getElementById("wrapper");
    component_box=document.getElementById("component_box");
    processBar=document.getElementById("processBar");
    processPoint=document.getElementById("processPoint");
    /**
     * 拖动裁剪框的逻辑处理。
     * */
